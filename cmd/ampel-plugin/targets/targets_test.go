...
// --- LoadTargetsByID tests ---

func TestLoadTargetsByID_ValidConfig(t *testing.T) {
	config, err := LoadTargetsByID("testdata/id-keyed-targets.yaml")
	require.NoError(t, err)
	require.Len(t, config.Targets, 2)

	githubEntry, ok := config.Targets["github-repos"]
	require.True(t, ok)
	require.Len(t, githubEntry.Repositories, 2)
	require.Equal(t, "https://github.com/myorg/repo1", githubEntry.Repositories[0].URL)
	require.Equal(t, []string{"main", "release"}, githubEntry.Repositories[0].Branches)

	internalEntry, ok := config.Targets["internal-repos"]
	require.True(t, ok)
	require.Len(t, internalEntry.Repositories, 1)
}

func TestLoadTargetsByID_MissingFile(t *testing.T) {
	_, err := LoadTargetsByID("testdata/nonexistent.yaml")
	require.Error(t, err)
	require.Contains(t, err.Error(), "reading targets file")
}

func TestLoadTargetsByID_EmptyTargets(t *testing.T) {
	tmpDir := t.TempDir()
	path := tmpDir + "/empty.yaml"
	writeTestFile(t, path, []byte("targets: {}"))
	_, err := LoadTargetsByID(path)
	require.Error(t, err)
	require.Contains(t, err.Error(), "no targets")
}

func TestLoadTargetsByID_NoRepositories(t *testing.T) {
	tmpDir := t.TempDir()
	path := tmpDir + "/no-repos.yaml"
	content := `targets:
  github-repos:
    repositories: []
`
...
func TestLoadTargetsByID_InvalidRepoURL(t *testing.T) {
	tmpDir := t.TempDir()
	path := tmpDir + "/bad-url.yaml"
	content := `targets:
  github-repos:
    repositories:
      - url: not-a-valid-url
...
func TestLoadTargetsByID_EmptyBranches(t *testing.T) {
	tmpDir := t.TempDir()
	path := tmpDir + "/no-branches.yaml"
	content := `targets:
  github-repos:
    repositories:
      - url: https://github.com/myorg/repo1
...
func TestLoadTargetsByID_MalformedYAML(t *testing.T) {
	tmpDir := t.TempDir()
	path := tmpDir + "/bad.yaml"
	writeTestFile(t, path, []byte("{ invalid yaml: [["))
	_, err := LoadTargetsByID(path)
