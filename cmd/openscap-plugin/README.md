...
| Requirement | Purpose |
|:---|:---|
| Go 1.24+ | Build `complyctl` and `openscap-plugin` |
| `openscap-scanner` | Provides the `oscap` CLI used by the plugin |
| `scap-security-guide` | Provides SCAP Datastream XML files in `/usr/share/xml/scap/ssg/content/` |
| RHEL, CentOS, or Fedora | SSG datastreams are distribution-specific; the plugin auto-detects via `/etc/os-release` |
...
| `openscap-plugin` | `bin/openscap-plugin` |

## Step 2: Install the OpenSCAP Plugin

The plugin discovery mechanism scans `~/.complytime/providers/` for executables matching `complyctl-provider-<evaluator-id>`. The build outputs `openscap-plugin` — it must be copied with the correct name.

```bash
mkdir -p ~/.complytime/providers
cp bin/openscap-plugin ~/.complytime/providers/complyctl-provider-openscap
...
```bash
# The plugin auto-detects this, but you need it to seed the registry
ls /usr/share/xml/scap/ssg/content/ssg-*-ds.xml
```
...
**Important:** All entries share the same `parameters` values (except `control_id`). The plugin merges parameters from all assessment configurations, and later entries override earlier ones for the same key.
