...
- **FR-013**: All Go exported and unexported identifiers in `pkg/plugin/` (to be renamed `pkg/provider/`) that use "Plugin" in their name MUST be renamed to use "Provider" — including handshake-related constants (e.g., `PluginHandshake` → `ProviderHandshake`). The exception is identifiers whose names are dictated by the `hashicorp/go-plugin` library interface (e.g., method names on `goplugin.Plugin`). The string *values* of handshake constants (the magic cookie key `"COMPLYCTL_PLUGIN"` and its UUID value) MUST remain unchanged to preserve runtime compatibility with existing installed provider binaries.
...
- The `hashicorp/go-plugin` handshake string values (magic cookie key `"COMPLYCTL_PLUGIN"`, its UUID value, and protocol version) are treated as a stable wire contract and MUST NOT change as part of the terminology rename. Go constant *identifiers* that carry these values (e.g., `PluginHandshake`) MUST be renamed to use "Provider" terminology (e.g., `ProviderHandshake`), but only the identifier name changes — the underlying string values remain byte-for-byte identical.
...
- The `complytime-providers` repository fork is already available locally at `/home/maburgha/GIT/complytime-providers/` and contains only a stub (`LICENSE`, `README.md`) — all provider source code migration is new work.
